...
export function Importacao() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // indicadores_fonte: tabela de relacionamento entre indicadores e fontes
      // Assume estrutura: { id_indicador, id_fonte } com join em indicadores
      supabase.from('indicadores_fonte').select('*, indicadores(*)'),
    ]).then((res) => {
      // Agrupa indicadores por id_fonte para filtragem rápida
      const indFonteRows: any[] = res[7].data || []
      const indicadoresFonte: Record<number, any[]> = {}
      indFonteRows.forEach((row) => {
        const fid = row.id_fonte
        if (!indicadoresFonte[fid]) indicadoresFonte[fid] = []
        if (row.indicadores) indicadoresFonte[fid].push(row.indicadores)
      })
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="border p-4 rounded-lg bg-slate-50 space-y-4">
              ...
              {!idFonte ? (
                ...
                <p className="text-xs text-muted-foreground">
                  Nenhum indicador disponível para esta fonte.
                </p>
              ) : (
                <div className="space-y-2">
