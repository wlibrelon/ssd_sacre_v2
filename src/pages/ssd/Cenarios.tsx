...
import {
  DadosGeraisTab,
  DadosPorFonteTab,
  CaptacaoDistribuicaoTab,
  FinanceiroTab,
} from './components/CenariosDashboard'
...
// ── Campos do Quadro de Parâmetros do Modelo ────────────────────────────────────
// Declarados FORA do componente Cenarios para preservar o foco do <Input>
// entre re-renders.
function SimField({
  label,
  field,
...
export default function Cenarios() {
  ...
  // Lista completa de indicadores aplicados — exibidos sempre que existirem,
  // sem necessidade de seleção manual pelo usuário.
  ...
  // Carrega os indicadores aplicados à simulação — sem seleção manual,
  // todos os indicadores existentes serão exibidos automaticamente nos
  ...
  // Gráficos de indicadores extras — geram-se para TODOS os indicadores
  // aplicados, sem depender de seleção manual.
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      ════════════════════════════════════════════════════════════════════ */}
      <Accordion type="multiple" defaultValue={['configuracoes']} className="space-y-4">
        <AccordionItem value="configuracoes" className="border rounded-xl bg-white shadow-sm">
          ...
          <AccordionContent className="px-5 pb-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="border p-4 rounded-lg bg-white space-y-3">
                        ...
                        <Button
                          ...
                          disabled={!idTc || !idC}
                          onClick={() => {
                            const t = refData.tiposCenario.find(
                              (x: any) => x.id_tc.toString() === idTc,
                            )
                        ...
                        <ul className="space-y-1.5">
                          {draftCenarios.map((c, i) => (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  setDraftCenarios((p) => p.filter((_, idx) => idx !== i))
                                }
              ...
              <TabsContent value="automaticos" className="space-y-4 pt-4">
                ...
                {!temAutomatico && (
                  <p className="text-xs text-muted-foreground py-4 text-center">
                    Ative "Demanda automática" ou "Perdas automáticas" na aba Parâmetros do Modelo
            ...
            {/* Botão Executar Simulação — fixo dentro do accordion de Configurações */}
        ...
        {/* ════════════════════════════════════════════════════════════════
            ACCORDION: Resultados (só aparece após executar com dados)
        ════════════════════════════════════════════════════════════════ */}
        {ran && data.length > 0 && (
          <AccordionItem value="resultados" className="border rounded-xl bg-white shadow-sm">
            <AccordionTrigger className="px-5 py-4 hover:no-underline">
              <span className="font-semibold text-primary text-base">Resultados</span>
            </AccordionTrigger>
            <AccordionContent className="px-5 pb-5">
              <Tabs defaultValue="dados_gerais" className="w-full">
                <TabsList className="flex-wrap h-auto">
                  <TabsTrigger value="dados_gerais">Dados Gerais</TabsTrigger>
                  <TabsTrigger value="seguranca_hidrica">Índice de Segurança Hídrica</TabsTrigger>
                  <TabsTrigger value="por_fonte">Dados por Fonte</TabsTrigger>
                  <TabsTrigger value="captacao_distribuicao">Captação e Distribuição</TabsTrigger>
                  <TabsTrigger value="indicadores" disabled={indicadoresCharts.length === 0}>
                    Indicadores
                  </TabsTrigger>
                  <TabsTrigger value="financeiro">Financeiro</TabsTrigger>
                  <TabsTrigger value="projecao_populacional" disabled={!temAutomatico}>
                    Projeção Populacional
                  </TabsTrigger>
                </TabsList>

                {/* ── TAB: Dados Gerais ── */}
                <TabsContent value="dados_gerais" className="pt-4">
                  <DadosGeraisTab data={data} fontesMap={fontesMap} />
                </TabsContent>

                {/* ── TAB: Índice de Segurança Hídrica ── */}
                <TabsContent value="seguranca_hidrica" className="space-y-6 pt-4">
                          ...
                          <div className="flex items-center justify-between border-b pb-3 mb-4">
                            <div>
                              ...
                              <p className="text-[11px] text-slate-400 mt-0.5">
                                Células marcadas indicam meses com déficit hídrico — passe o mouse
                                para ver o valor
                              </p>
                            </div>
                            <button
                            ...
                            {(['alerta', 'crise', 'colapso'] as StatusSeg[]).map((s) => (
                              <div key={s} className="flex items-center gap-1.5">
                                <span
                                  ...
                                  style={{
                                    backgroundColor: cellCss[s].bg,
                                    borderColor: cellCss[s].dot,
                                  }}
                                />
                ...
                {/* ── TAB: Dados por Fonte ── */}
                <TabsContent value="por_fonte" className="pt-4">
                  <DadosPorFonteTab data={data} fontesMap={fontesMap} />
                </TabsContent>

                {/* ── TAB: Captação e Distribuição ── */}
                <TabsContent value="captacao_distribuicao" className="pt-4">
                ...
                <TabsContent value="indicadores" className="space-y-6 pt-4">
                  {indicadoresCharts.length > 0 ? (
                    ...
                    <p className="text-xs text-muted-foreground py-4 text-center">
                      Nenhum indicador aplicado para esta simulação.
                    </p>
                  )}
                </TabsContent>
                ...
                {/* ── TAB: Projeção Populacional ── */}
                <TabsContent value="projecao_populacional" className="pt-4">
                  {temAutomatico ? (
                    (() => {
                    ...
                    <p className="text-xs text-muted-foreground py-4 text-center">
                      Ative "Demanda automática" ou "Perdas automáticas" para visualizar a projeção
                      populacional.
                    </p>
                  )}
                </TabsContent>
